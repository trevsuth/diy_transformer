...
# Hugingface datasets and tokenizers
...
def get_ds(config):
    ...
    # Find the maximim length of each sentence in the source and target sentence
    max_len_src = 0
    max_len_tgt = 0
...
def get_model(config, vocab_src_len, vocab_tgt_len):
    model = build_transformer(
        vocab_src_len, vocab_tgt_len, config["seq"], d_model=config["d_model"]
    )
    return model
...
def train_model(config):
    ...
    # If the user specified a model to preload prior to training, load it
    initial_epoch = 0
    global_step = 0
    ...
    if model_filename:
        ...
        model.load_state_dict(state["optimizer_state_dict"])
    ...
    for epoch in range(initial_epoch, config["num_epochs"]):
        ...
        # Run validation that the end of every epoch
        run_validation(
            model,
            val_dataloader,
            tokenizer_src,
